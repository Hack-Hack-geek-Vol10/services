...
func NewConnection() Connection {
	...
	connectAttempts := config.Config.Database.ConnectAttempts
	if connectAttempts == 0 {
		connectAttempts = 1
	}

	connectWaitTimeSeconds := config.Config.Database.ConnectWaitTime
	...
	conn := &pqDatabaseConnection{
		...
		connectBlocks:          config.Config.Database.ConnectBlocks,
